...
class Trainer(object):
    def __init__(self, args):
        ...
        # create teacher
        self.loss_func = DistributionLoss().to(args.device)
        ...
        self.model_teacher = get_model(args)
        ...
        # optimizer
        self.optimizer = get_optimizer(args, self.model.parameters(), None, None)
        self.lr_scheduler = LRScheduler(mode=args.lr_mode, base_lr=args.lr, nepochs=args.epochs,
        ...
        for epoch in range(self.args.start_epoch, self.args.epochs):
            ...
            try: 
                ...
                for i, (images, targets) in enumerate(self.train_loader):
                    ...
                    self.lr_scheduler.update_lr(self.optimizer, cur_iter)

                    self.reg.pre_update()

                    logits_student = self.model(images)
                    outputs = logits_student
                    logits_teacher = self.model_teacher(images).detach()
                    loss = self.loss_func(logits_student, logits_teacher)
                    ...
                    if cur_iter % 20 == 0:
                        if not self.reg.binary_params:
                            ...
                            print('Epoch: [{e_}/{E_}] Iter [{iter_}/{I_}] || '
                              ...
                              lr_=self.lr_scheduler.learning_rate,
                              loss_=loss.item(), 
            ...
            if self.args.no_val: # save every epoch
                ...
                if self.args.reg_type == 'ternary' or self.args.reg_type == 'l05_ternary' or self.args.reg_type == 'our_binary' or self.args.reg_type == 'binary' or self.args.reg_type == 'l1_binary' or self.args.reg_type == 'l2_binary':
                    if self.reg.epoch in args.freeze_epoch:
                        self.validation(epoch, None, self.val_loader)
                    else:
                        self.reg.print_mean_dist(quantised=False)
                        print('please delete')
                        self.metric_1, self.metric_2 = 0.0, 0.0
                else:
                    self.validation(epoch, None, self.val_loader)
            ...
            if self.summary_writer is not None:
                ...
                self.summary_writer.add_scalar('opt/lr', float(self.lr_scheduler.learning_rate), epoch)
    ...
    def validation(self, epoch, quant_function, loader):
        ...
        if is_quant:
            if self.metric_1 > self.best_quant:
                ...
                save_checkpoint(self.model, self.optimizer, self.args, is_best=True, is_quantised=is_quant)
...
if __name__ == '__main__':
    ...
    with mlogger.stdout_to("{}/log.txt".format(args.xp_name), enabled=not(args.debug)):
        ...
        if args.eval:
            ...
            #trainer.validation(args.start_epoch, quant_func, trainer.val_loader)
            #trainer.create_summary(args.resume, 'val'+tag, is_quant)
            ...
            trainer.train()
            if args.reg_type == 'none' or args.reg_type == 'convex_hull' or 'our' in args.reg_type or 'l1' in args.reg_type or 'l2' in args.reg_type:
                trainer.load_best_model(False)
            else:
                trainer.load_best_model(True)
